...
const token = process.env.MODI_GITHUB_TOKEN
...
function zipAssets () {
  ...
  const zipAssets = [{
    name: 'modi-mac.zip',
    path: path.join(outPath, 'MODI-darwin-x64', 'MODI.app')
  }, {
    name: 'modi-windows.zip',
    path: path.join(outPath, 'MODI-win32-ia32')
  }, {
    name: 'modi-linux.zip',
    path: path.join(outPath, 'MODI-linux-x64')
  }]
  ...
  return Promise.all(zipAssets.map(zipAsset)).then((zipAssets) => {
    return zipAssets.concat([{
      ...
      name: 'MODISetup.exe',
      path: path.join(outPath, 'windows-installer', 'MODISetup.exe')
    }, {
      name: `MODI-${version}-full.nupkg`,
      path: path.join(outPath, 'windows-installer', `MODI-${version}-full.nupkg`)
    }])
  })
}
...
function createRelease (assets) {
  const options = {
    uri: 'https://api.github.com/repos/OUsrt/modi/releases',
    headers: {
